...
**omitted entirely** from the output when no source contributed a value —
not `null`, not an empty object, simply absent. `jq '.expires.value'` on a
...
- **`lifecycle`** — plat's own interpretation of where an expired gTLD domain sits in ICANN's Expired Domain Deletion Policy timeline, derived from `status`/`updated`/`expires` rather than reported directly by any source. Omitted for ccTLDs (which set independent policies this schema doesn't model) and for domains with no recognized lifecycle-relevant status:
  ```json
  {
    ...
    "estimateBasis": "Estimate based on ICANN's 30-day Redemption Grace Period policy for gTLDs, calculated from this record's last-updated time. Actual timing is set by the registry/registrar and may be earlier."
  }
  ```
